...
#### 6.2 個人設定（グローバル設定）
すべての交渉に適用されるデフォルト設定。アカウント設定 > メール通知設定で管理。

##### 通知カテゴリ
| カテゴリ | 説明 | デフォルト |
|----------|------|-----------|
| **相手先からのアクション** | 契約書の更新、コメント、署名リクエストなど。編集とコメントが同時の場合は1通にまとめて配信 | 受け取る |
| **期限のリマインド** | 相手先との交渉で設定した期限が近づくとお知らせ。※契約台帳に直接登録した契約は対象外 | 受け取る |
| **契約更新・解約通知期限お知らせメール** | 毎月1日配信。更新・解約通知期限が迫っている契約書の件数をお知らせ（契約台帳機能） | 受け取る |
...
| 設定項目 | デフォルト値 | 選択肢 |
|----------|-------------|--------|
| リマインドタイミング | 1週間前 | 4週間前、2週間前、1週間前、3日前、1日前 |

#### 6.3 交渉ごとの通知設定
管理タブで設定。シンプルに「重要かどうか」のみ。

##### 重要な交渉としてマーク
| 項目 | 説明 |
|------|------|
| 効果 | 通知メールの件名に **[重要]** ラベルが付く |
| 用途 | 大型案件、期限が厳しい交渉など、メールボックスで見落としたくない場合に使用 |
...
#### 6.5 通知の種類一覧
| 通知タイプ | トリガー | 相手先からのアクション | 期限のリマインド |
|-----------|---------|:--------------------:|:---------------:|
| 契約書更新 | 相手が編集・コメント | ✅ | - |
| 署名リクエスト | 署名を求められた | ✅ | - |
| 合意完了 | 両者が合意し契約成立 | ✅ | - |
...
※「重要な交渉としてマーク」をONにすると、すべての通知の件名に [重要] が付く
