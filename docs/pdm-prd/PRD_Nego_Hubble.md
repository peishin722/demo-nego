...
- **バージョン同期** - Nego→Hubbleは自動同期、Hubble→Negoは「交渉相手に共有」で手動共有（§7参照）
...
### デモ操作ボタン（negotiation.html 左下）

| ボタン | 機能 |
|--------|------|
| 🟠 Hubble連携を切替 | 3段階サイクルで切り替え: ①連携済み（「Hubbleで見る」ボタン表示）→ ②未連携／Hubble登録あり（「Hubble連携」ボタン表示、連携モーダルを体験可）→ ③Hubble未登録（Hubble関連ボタン非表示）→ ①に戻る |
| 🟣 ステータスをデモ | 交渉ステータス（交渉中→署名送付済み→署名者ビュー→締結済み等）を順に切り替え |
